import reducer_0 from './reducer_0';
import reducer_1 from './reducer_1';
import reducer_2 from './reducer_2';
// import reducer_3 from './reducer_3';
// import reducer_4 from './reducer_4';
// import reducer_5 from './reducer_5';
// import reducer_6 from './reducer_6';
// import reducer_7 from './reducer_7';
export default function () {
    return [
        [
            {
                numTopic: 0,
                numQuestion: 1,
                question: 'Перше питання - Перша тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            },
            {
                numTopic: 0,
                numQuestion: 2,
                question: 'Друге питання - Перша тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            }
        ],
        [
            {
                numTopic: 1,
                num: 1,
                question: 'Перше питання   Друга тема',
                topic: 'Друга тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            },
            {
                numTopic: 1,
                num: 2,
                question: 'Друге питання   Друга тема',
                topic: 'Друга тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            }
        ],
        [
            {
                numTopic: 2,
                numQuestion: 1,
                question: 'Перше питання - 3 тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            },
            {
                numTopic: 2,
                num: 2,
                numQuestion: 'Друге питання - 3 тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            }
        ],
        [
            {
                numTopic: 3,
                numQuestion: 1,
                question: 'Перше питання - 4 тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            },
            {
                numTopic: 3,
                numQuestion: 2,
                question: 'Друге питання - 4 тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            }
        ],
        [
            {
                numTopic: 4,
                numQuestion: 1,
                question: 'Перше питання - 5 тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            },
            {
                numTopic: 4,
                numQuestion: 2,
                question: 'Друге питання - 5а тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            }
        ],
        [
            {
                numTopic: 5,
                numQuestion: 1,
                question: 'Перше питання - 6а тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            },
            {
                numTopic: 5,
                numQuestion: 2,
                question: 'Друге питання - 6а тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            }
        ],
        [
            {
                numTopic: 6,
                numQuestion: 1,
                question: 'Перше питання - 6а тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            },
            {
                numTopic: 6,
                numQuestion: 2,
                question: 'Друге питання - 6а тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            }
        ],
        [
            {
                numTopic: 7,
                numQuestion: 1,
                question: 'Перше питання - 7а тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            },
            {
                numTopic: 7,
                numQuestion: 2,
                question: 'Друге питання - 7а тема',
                topic: 'Перша тема',
                optionsAnswers: [
                    {
                        numOption: 'А',
                        text: "Перша відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Б',
                        text: "Друга відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'В',
                        text: "Третя відповідь",
                        isTrue: false
                    },
                    {
                        numOption: 'Г',
                        text: "Четверта відповідь",
                        isTrue: true
                    },
                ]
            }
        ]
    ]
};

